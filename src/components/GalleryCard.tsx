...
export function GalleryCard({ item, index }: { item: GalleryItem; index: number }) {
  ...
  return (
    <div className="group relative aspect-[3/4] overflow-hidden rounded-2xl border border-white/10 bg-[#24113D]">
      {item.image ? (
        <Image
          src={item.image}
          alt={item.caption}
        ...
        <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-[#24113D] to-[#0E0B12]">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent p-4">
        <p className="mb-3 text-sm font-medium text-[#F4EBDD]">{item.caption}</p>
        <a
          href={buildWhatsAppLink(`Hola Lucio, vi "${item.caption}" y quiero agendar una consulta.`)}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-1.5 rounded-full bg-[#D4AF37] px-3.5 py-2 text-xs font-semibold text-[#0E0B12] transition-opacity hover:opacity-90"
        >
